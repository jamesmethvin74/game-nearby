(() => {
  ...
  const statusFor = (event) => (
    typeof getTeamStatus === "function"
      ? getTeamStatus(event)
      : { overall: "—", conference: "—", standing: "Not posted", conferenceName: "Conference" }
  );

  const miniStatus = (status) => `
    <div class="mini-status" aria-label="Team record and standing">
      <span>${status.overall} overall</span>
      <span>${status.conference} conf.</span>
