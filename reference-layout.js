(() => {
  ...
  const timingLabel = (event) => {
    const date = new Date(event.date), today = new Date();
  ...
  const mascotBadge = (event) => {
    ...
    if (mascot.mark) return `<div class="team-badge team-mascot mascot-${mascot.className}"><img class="team-mark-img" src="${mascot.mark}" alt="${mascot.label} logo" /></div>`;
    return `<div class="team-badge team-mascot mascot-${mascot.className}" role="img" aria-label="${mascot.label} mascot">${mascot.icon}</div>`;
  };

  const statusFor = (event) => typeof getTeamStatus === "function"
    ? getTeamStatus(event)
