(() => {
  ...
  function resolvedWidgetId() {
    ...
    try { return validWidgetId(localStorage.getItem(WIDGET_STORAGE_KEY)); } catch { return ""; }
  }
    ...
    host.innerHTML = `
      <div class="scorestream-empty">
        <div class="scorestream-empty-inner">
          <h2>Arkansas live scores are ready to plug in</h2>
          <p>LocalBleachersAR can use ScoreStream's official, free scoreboard widget here. No scraping and no production D1 writes are involved. Until a free widget ID is configured, you can open ScoreStream's public Arkansas football scoreboard directly.</p>
          <div class="scorestream-actions">
            ...
            <a class="scorestream-btn" href="https://scorestream.com/make-a-scoreboard-widget" target="_blank" rel="noopener noreferrer">Create free widget</a>
          </div>
        </div>
      </div>`;
  }
  ...
  function loadWidget(widgetId) {
    ...
    script.onload = () => {
      if (status) status.textContent = `Official ScoreStream widget #${widgetId}`;
    };
    script.onerror = () => {
      if (error) {
        error.hidden = false;
  ...
  const widgetId = resolvedWidgetId();
  if (widgetId) loadWidget(widgetId);
  else showFallback();
})();
