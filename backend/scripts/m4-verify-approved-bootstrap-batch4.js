...
if (payload.status !== "SUCCESS" && payload.status !== "SKIPPED") {
  throw new Error(`Unexpected M4 Batch 4 status: ${JSON.stringify(payload)}`);
}
if (selected < 0 || selected > 8 || attempted < 0 || attempted > 8) {
  throw new Error(`M4 Batch 4 exceeded batch bound: ${JSON.stringify({ selected, attempted })}`);
}
if (payload.status === "SUCCESS" && selected < 1) {
  throw new Error(`M4 Batch 4 reported SUCCESS without selecting a source: ${JSON.stringify(payload)}`);
}
if (payload.status === "SKIPPED" && (selected !== 0 || attempted !== 0)) {
  throw new Error(`M4 Batch 4 SKIPPED with unexpected work: ${JSON.stringify({ selected, attempted, payload })}`);
}
...
if (hardFailures.length) {
  throw new Error(`M4 Batch 4 stopped on hard failure: ${JSON.stringify(hardFailures)}`);
}

if (payload.status === "SUCCESS" && after.populatedSources <= before.populatedSources) {
  throw new Error(`M4 Batch 4 produced no new populated source: ${JSON.stringify({ before, after, payload })}`);
}
if (after.gameRows < before.gameRows) {
  throw new Error(`M4 Batch 4 game-row count regressed: ${JSON.stringify({ before, after })}`);
}

console.log(JSON.stringify({
  status: "M4_BOOTSTRAP_BATCH4_VERIFIED",
