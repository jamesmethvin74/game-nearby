...
TRIGGER_URL="https://localbleachersar-sports-api.james-methvin74.workers.dev/api/v1/m4/bootstrap-approved-b4"
BEFORE_JSON="/tmp/m4-batch4-before.json"
PAYLOAD_JSON="/tmp/m4-batch4-payload.json"
AFTER_JSON="/tmp/m4-batch4-after.json"
...
printf '%s' "$TOKEN" | wrangler secret put M4_BATCH4_TOKEN

READY_STATUS=""
for ATTEMPT in 1 2 3 4 5 6 7 8; do
  READY_STATUS="$(curl -sS -o /dev/null -w '%{http_code}' --max-time 20 --head \
    -H "x-m4-batch4-token: $TOKEN" \
    -H 'cache-control: no-store' \
    "$TRIGGER_URL" || true)"
  if [ "$READY_STATUS" = "204" ]; then
    echo "M4_BATCH4_READY attempt=$ATTEMPT"
    break
  fi
  echo "Batch 4 readiness attempt $ATTEMPT returned HTTP ${READY_STATUS:-curl_error}" >&2
  sleep 3
done

if [ "$READY_STATUS" != "204" ]; then
  echo "Batch 4 readiness never reached 204; last status=${READY_STATUS:-curl_error}" >&2
  exit 1
fi
...
if [ "$POST_STATUS" != "200" ]; then
  echo "Batch 4 POST failed: HTTP $POST_STATUS" >&2
  cat "$PAYLOAD_JSON" >&2 || true
  exit 1
fi

wrangler d1 execute localbleachersar-sports --remote --command="$STATE_SQL" --json > "$AFTER_JSON"
node scripts/m4-verify-approved-bootstrap-batch4.js "$BEFORE_JSON" "$PAYLOAD_JSON" "$AFTER_JSON"
