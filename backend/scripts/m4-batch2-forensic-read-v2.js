...
WHERE sch.level='college'
  ...
  AND t.season='2026'
  AND cr.started_at >= '2026-09-04T15:32:40Z'
  AND cr.started_at <  '2026-09-04T15:33:20Z'`;
...
if (!row) throw new Error("Batch 2 forensic query returned no row");
...
console.log(JSON.stringify({ status:"M4_BATCH2_FORENSIC", ...result }));

// For this forensic build, success means the failed Batch 2 execution never
// reached collection. Any collection row causes a deliberate failed check so
// we do not retry blindly.
if (result.collectionRuns !== 0) {
  throw new Error(`Batch 2 reached collection: ${JSON.stringify(result)}`);
}
