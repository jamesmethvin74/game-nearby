...
GAME_SQL="WITH candidate AS (
  SELECT g.source_id,g.scheduled_at,g.scheduled_time_known,g.venue,g.location_text,g.latitude,g.longitude,g.home_away,
  ...
  WHERE bt.sport='football' AND bt.gender='boys' AND bt.season='2026'
    AND lower(replace(bs.name,'-',' ')) LIKE 'blevins%'
    ...
    AND src.parser_type='hootens-statewide'
), target AS (
  ...
  WHERE t.sport='football' AND t.gender='boys' AND t.season='2026'
    AND lower(replace(s.name,'-',' ')) LIKE 'guy%perkins%'
), vals AS (
  SELECT target.team_id,target.school_id,candidate.*,
         'mirror:guy-perkins-blevins:'||substr(candidate.scheduled_at,1,10) AS event_key
...
SELECT team_id||'-'||event_key,team_id,source_id,event_key,blevins_name,blevins_school_id,
       ...
       'Hooten statewide final mirrored from Blevins result for Guy-Perkins schedule completion',source_url,
...
WHERE NOT EXISTS(
  ...
  WHERE g.team_id=vals.team_id AND g.status='FINAL' AND g.team_score=28 AND g.opponent_score=12
    AND datetime(g.scheduled_at)>=datetime('now','-168 hours')
    AND (g.opponent_school_id=vals.blevins_school_id OR lower(replace(COALESCE(g.opponent,''),'-',' ')) LIKE 'blevins%')
)"
...
RECORD_SQL="WITH target AS (
  ...
  SELECT target.team_id,
    COALESCE(SUM(CASE WHEN g.status='FINAL' AND COALESCE(g.counts_for_record,1)=1 AND g.result='W' THEN 1 ELSE 0 END),0) wins,
    COALESCE(SUM(CASE WHEN g.status='FINAL' AND COALESCE(g.counts_for_record,1)=1 AND g.result='L' THEN 1 ELSE 0 END),0) losses,
    COALESCE(SUM(CASE WHEN g.status='FINAL' AND COALESCE(g.counts_for_record,1)=1 AND g.result='T' THEN 1 ELSE 0 END),0) ties,
    COALESCE(SUM(CASE WHEN g.status='FINAL' AND COALESCE(g.counts_for_record,1)=1 AND g.conference_game=1 AND g.result='W' THEN 1 ELSE 0 END),0) conference_wins,
    COALESCE(SUM(CASE WHEN g.status='FINAL' AND COALESCE(g.counts_for_record,1)=1 AND g.conference_game=1 AND g.result='L' THEN 1 ELSE 0 END),0) conference_losses,
    COALESCE(SUM(CASE WHEN g.status='FINAL' AND COALESCE(g.counts_for_record,1)=1 AND g.conference_game=1 AND g.result='T' THEN 1 ELSE 0 END),0) conference_ties
  FROM target LEFT JOIN games g ON g.team_id=target.team_id
  GROUP BY target.team_id
)
INSERT OR REPLACE INTO team_records(team_id,wins,losses,ties,conference_wins,conference_losses,conference_ties,calculated_at)
SELECT team_id,wins,losses,ties,conference_wins,conference_losses,conference_ties,CURRENT_TIMESTAMP FROM calc"
...
if(finals<90||matched!==finals||unmatched!==0||found!==10||mask!==1023||String(r.conway_status)!=='FINAL'||cs!==14||bs!==20||writes!==0){
  throw new Error(`HOOTENS_FINAL_VERIFY_FAILED ${JSON.stringify({finals,matched,unmatched,found,mask,conwayStatus:r.conway_status,cs,bs,writes})}`);
}
console.log(`hv-f${finals}m${matched}u0-r${found}k${mask}-c${cs}x${bs}`.slice(0,32));
NODE
)"

echo "HOOTENS_GP_GAME_WRITES $GAME_WRITES"
