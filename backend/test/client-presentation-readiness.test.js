...
test("client release presents one factual display contract on all three surfaces", () => {
  ...
  assert.match(runtime, /m15-presentation-v80/);
  assert.match(runtime, /clientGeneration:\s*80/);
  assert.match(runtime, /shellGeneration:\s*80/);
  ...
  for (const html of [index, standingsHtml, teamsHtml]) {
    assert.match(html, /runtime-build\.js\?v=80/);
    assert.match(html, /service-worker\.js\?v=80/);
  ...
  assert.match(liveScoresHtml, /service-worker\.js\?v=80/);
  assert.match(liveScoresHtml, /updateViaCache:"none"/);
  assert.match(sw, /localbleachersar-shell-v80/);
