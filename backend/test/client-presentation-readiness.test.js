...
test("client release presents one factual display contract on all three surfaces", () => {
  ...
  assert.match(runtime, /m15-presentation-v79/);
  assert.match(runtime, /clientGeneration:\s*79/);
  assert.match(runtime, /shellGeneration:\s*79/);
  ...
  for (const html of [index, standingsHtml, teamsHtml]) {
    assert.match(html, /runtime-build\.js\?v=79/);
    assert.match(html, /service-worker\.js\?v=79/);
  ...
  assert.match(liveScoresHtml, /service-worker\.js\?v=79/);
  assert.match(liveScoresHtml, /updateViaCache:"none"/);
  assert.match(sw, /localbleachersar-shell-v79/);
