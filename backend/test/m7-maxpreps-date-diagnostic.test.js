...
test("MaxPreps date replay diagnostic is zero-D1 and detects repeated contest ids", async()=>{
  const pages={
  ...
  assert.equal(result.repeated_contests,1);
  assert.equal(result.duplicate_observations,1);
  assert.equal(result.requested_vs_url_date_mismatches,1);
});
