...
test("bounded convergence strips an unapproved same-day final before the writer sees the page",async()=>{
  ...
  const runner=async (_env,opts)=>{
    ...
    return {status:"SUCCESS",matchedFinals:3,observations:3,touchedTeams:3,writes:12,recordResult:{standings:{cohorts:0}}};
  };
  ...
  assert.equal(result.preflight.conferenceCohorts,0);
...
test("bounded convergence aborts before writer work if an approved contest disappears",async()=>{
  ...
  const runner=async (_env,opts)=>{
    ...
    writerReached=true;
    return {status:"SUCCESS",matchedFinals:0,observations:0,touchedTeams:0,recordResult:{standings:{cohorts:0}}};
  };
...
test("bounded convergence aborts if a target team gains conference membership",async()=>{
  const env={DB:fakeD1({conferenceMembership:true})};
  let runnerCalls=0;
  await assert.rejects(
    runVolleyballConvergenceBatch(env,{batchKey:VOLLEYBALL_CONVERGENCE_BATCH_KEY,runner:async()=>{runnerCalls++;}}),
    /target now has conference membership/
  );
  assert.equal(runnerCalls,0);
});
