...
import { buildVolleyballLiveCalculatedStandings, conferenceRecordsFromRosterFinals } from "../src/volleyball-standings-overlay.js";
...
test("volleyball snapshot decision ignores identical semantic feed and detects score changes", () => {
  ...
  assert.deepEqual(volleyballResultSnapshotChanged(JSON.stringify({ signature }), payload), { changed:false, previousSignature:signature, signature });
...
test("unchanged live volleyball probe performs no D1 writes", async () => {
  ...
  const env = { DB:{ prepare(sql){ assert.match(sql, /statewide_collection_state/); return { bind(){ return this; }, async first(){ reads += 1; return { details_json:JSON.stringify({ signature }) }; }, async run(){ runs += 1; throw new Error("unchanged probe must not write D1"); } }; } } };
  const fetchFn = async () => ({ ok:true, status:200, async json(){ return { ...payload, hasNextPage:false }; } });
  const result = await runVolleyballLiveResultProbe(env, { fetchFn, now:new Date("2026-09-02T23:00:00.000Z") });
...
test("conference records are derived from canonical finals between roster members", () => {
  const records = conferenceRecordsFromRosterFinals([{ home_school_id:"greenbrier", away_school_id:"vilonia", home_score:3, away_score:1 },{ home_school_id:"maumelle", away_school_id:"greenbrier", home_score:0, away_score:3 }]);
...
test("live volleyball records only advance published records and preserve the full roster", () => {
  ...
  const calculated = buildVolleyballLiveCalculatedStandings(published, [{ normalized_alias:"greenbrier", team_id:"greenbrier-volleyball-2026", wins:4,losses:2,ties:0, conference_wins:2,conference_losses:1,conference_ties:0, calculated_at:"2026-09-02T23:00:00.000Z" },{ normalized_alias:"vilonia", team_id:"vilonia-volleyball-2026", wins:3,losses:1,ties:0, conference_wins:1,conference_losses:0,conference_ties:0, calculated_at:"2026-09-02T23:00:00.000Z" }]);
  ...
  assert.equal(calculated.standings.some(row => row.school_name === "Vilonia"), false, "a less-complete local record must not replace a more-complete published record");
...
test("ambiguous multi-game conference inference does not overwrite the published conference record", () => {
  const published = publishedVolleyball();
  const calculated = buildVolleyballLiveCalculatedStandings(published, [{ normalized_alias:"greenbrier", team_id:"greenbrier-volleyball-2026", wins:6,losses:2,ties:0, conference_wins:4,conference_losses:1,conference_ties:0, calculated_at:"2026-09-02T23:00:00.000Z" }]);
  assert.ok(calculated, "overall record can still advance");
  assert.equal(calculated.standings[0].overall_record, "6-2");
  assert.equal(calculated.standings[0].conference_record, "1-1", "more than one inferred game ahead is ambiguous and must preserve published conference record");
});

test("standings GET path is read-only and delegates volleyball overlay through shared truth", async () => {
  const source = await readFile(new URL("../src/standings-worker.js", import.meta.url), "utf8");
  assert.match(source, /loadStandingsTruth/);
