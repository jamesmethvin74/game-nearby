...
async function extractScoreboardRows(html,HTMLRewriterClass=globalThis.HTMLRewriter){
  ...
  const response=new HTMLRewriterClass()
    .on("tr",{
      element(el){state.current={cells:["","","","",""],cellIndex:-1,teamLinks:[],activeLink:null};state.rows.push(state.current);el.onEndTag(()=>{state.current=null;});}
    })
    .on("tr td",{
      element(el){if(!state.current)return;state.current.cellIndex+=1;const idx=state.current.cellIndex;el.onEndTag(()=>{if(state.current&&state.current.cellIndex===idx){};});},
    ...
    .on("tr a[href*='/teams/']",{
      element(el){if(!state.current)return;const link={href:el.getAttribute("href")||"",text:""};state.current.teamLinks.push(link);state.current.activeLink=link;el.onEndTag(()=>{if(state.current)state.current.activeLink=null;});},
...
async function updateExternalAnchor(env,anchor,teamScore,opponentScore,checkedAt,scoreboardUrl){
  const result=resultCode(teamScore,opponentScore);
  await env.DB.prepare(`UPDATE games SET status='FINAL',team_score=?,opponent_score=?,result=?,notes=CASE WHEN notes IS NULL OR notes='' THEN ? ELSE notes END,last_checked_at=?,updated_at=? WHERE id=?`).bind(teamScore,opponentScore,result,`Final score verified from Hooten's statewide scoreboard: ${scoreboardUrl}`,checkedAt,checkedAt,anchor.id).run();
}
...
export async function runHootensStatewideResults(env,{fetchFn=fetch,HTMLRewriterClass=globalThis.HTMLRewriter,now=new Date(),force=false}={}){
  ...
  try{
    ...
    for(const final of finals){
      const home=pickSchool(final.homeName,aliasIndex),away=pickSchool(final.awayName,aliasIndex);
      const reporting=home||away;
      if(!reporting){unmatched.push({home:final.homeName,away:final.awayName,reason:"no_local_school_match"});continue;}
      const reportingIsHome=Boolean(home);
      const opponentLocal=reportingIsHome?away:home;
      const opponentName=reportingIsHome?final.awayName:final.homeName;
      const teamScore=reportingIsHome?final.homeScore:final.awayScore;
      const opponentScore=reportingIsHome?final.awayScore:final.homeScore;
      ...
      if(anchor.opponent_school_id){
        ...
        await updateExternalAnchor(env,anchor,teamScore,opponentScore,checkedAt,scoreboardUrl);
      }
