...
// Current Arkansas MaxPreps directory exposes 2A-6A plus additional associations.
// The legacy helper fallback has only six large-school conferences, so fail closed
// rather than presenting that fallback as a statewide audit.
const PUBLISHED_CONFERENCE_MINIMUM=20;

// Statement 1: one row per active local 2026 Arkansas varsity volleyball team.
// source_counts is deliberately set-based; do not replace it with per-team source queries.
const TEAM_SNAPSHOT_SQL=`
  ...
  source_counts AS (
    ...
    FROM sources src
    JOIN vb_teams vt ON vt.team_id=src.team_id
  ...
  SELECT vt.*,
    ...
    COALESCE(sc.enabled_source_count,0) AS enabled_source_count
  FROM vb_teams vt
  ...
  LEFT JOIN source_counts sc ON sc.team_id=vt.team_id
  ORDER BY vt.school_name,vt.team_id
`;

// Statement 2: unique canonical events touching the local volleyball cohort during
// the bounded fall season window, with set-based member/conflict aggregates.
...
// Statement 3: the same schedule/record candidates production uses. Canonical rows
// are collapsed to one candidate per reporting team/event; uncanonical rows remain
// visible so the audit can detect orphan finals and reproduce record dedupe semantics.
...
function recordText(wins=0,losses=0,ties=0){return Number(ties)?`${Number(wins)}-${Number(losses)}-${Number(ties)}`:`${Number(wins)}-${Number(losses)}`;}
...
function localDateOffset(value,days){
  ...
  return localDate(new Date(Date.UTC(year,month-1,day+days,18,0,0)));
}
...
function pairDateKey(a,b,date){return `${pairKey(a,b)}|${date||""}`;}
function scoreMap(row){return new Map([[row.home_school_id,Number(row.home_score)],[row.away_school_id,Number(row.away_score)]]);}
function canonicalMatchesAuthority(event,final){
  ...
  const expected=new Map([[final.homeTeam.school_id,Number(final.home.score)],[final.awayTeam.school_id,Number(final.away.score)]]);
...
function aliasesForTeam(team){return [...new Set([team.school_name,team.raw_school_name,team.location_matched_name].map(normalizeSchoolAlias).filter(Boolean))];}
function teamAliasIndex(teams){
  const map=new Map();
  for(const team of teams) for(const alias of aliasesForTeam(team)){
...
function exceptionRow({team=null,conference=null,type,localState,authorityState,recommendedRepair,details=null}){
  return {
    ...
    deficiency_type:type,local_state:localState,authority_state:authorityState,
    recommended_repair:recommendedRepair,...(details?{details}:{})
  };
}
...
function expectedLocalTeamBySchool(teams){return new Map(teams.map(team=>[team.school_id,team]));}
function normalizeCandidate(row){return {...row,counts_for_record:Number(row.counts_for_record||0),conference_game:Number(row.conference_game||0),team_score:row.team_score==null?null:Number(row.team_score),opponent_score:row.opponent_score==null?null:Number(row.opponent_score)};}
...
export function buildVolleyballCompletenessAudit({teams=[],canonicals=[],candidates=[],published=null,maxPreps=null,generatedAt=new Date().toISOString()}={}){
  ...
  for(const team of teams){
    ...
    const audit={
      ...
      record_exists:Number(team.record_exists||0)>0,overall_record:storedOverall,conference_record:storedConference,
    ...
    if(audit.record_exists&&(storedOverall!==rebuiltOverall||storedConference!==rebuiltConference)) exceptions.push(exceptionRow({team,type:"record_rebuild_mismatch",localState:`stored ${storedOverall} (${storedConference} conf)`,authorityState:`canonical recompute ${rebuiltOverall} (${rebuiltConference} conf)`,recommendedRepair:"trace record-eligible canonical/raw finals; fix reconciliation before scoped rebuild"}));
  }
  ...
  for(const [key,events] of canonicalByPairDate){
    ...
    for(const schoolId of new Set(events.flatMap(event=>[event.participant_a_school_id,event.participant_b_school_id]))){
      ...
      if(team) exceptions.push(exceptionRow({team,type:"duplicate_canonical_event",localState:`${events.length} canonical events for ${key}`,authorityState:"one canonical event expected for the same participant pair/date unless proven separate matches",recommendedRepair:"canonical reconciliation/deduplication review",details:{canonical_event_ids:events.map(event=>event.canonical_event_id)}}));
    }
  }
  ...
  for(const event of canonicals){
    ...
    if(isFinal&&localParticipants.length>=2){
      for(const team of localParticipants) if(!reportingIds.has(team.team_id)) exceptions.push(exceptionRow({team,type:"cross_surface_missing_team_attachment",localState:`canonical final ${event.canonical_event_id} is not attached to this team's schedule`,authorityState:"Scores sees canonical; Team Schedule should attach the same canonical",recommendedRepair:"repair canonical_event_members/reconciliation attachment"}));
      if(Number(event.member_observation_count||0)===0) for(const team of localParticipants) exceptions.push(exceptionRow({team,type:"home_scores_visibility_gap",localState:"canonical final has no member game observation",authorityState:"Scores can see canonical but Home reads through attached games",recommendedRepair:"repair canonical member attachment, not read-path masking"}));
    }
  }
  ...
  if(published){
    for(const conference of published.conferences||[]){
      ...
      for(const row of conference.standings||[]){
        ...
        if(matches.length===0){
          unmatched++;
          exceptions.push(exceptionRow({conference:{...conference,id:expectedConferenceId},type:"published_roster_unmatched",localState:"no unique local varsity volleyball team",authorityState:`published ${row.school_name} ${row.overall_record} (${row.conference_record} conf)`,recommendedRepair:"catalog/team identity review before membership materialization",details:{school_name:row.school_name,conference_id:expectedConferenceId,conference_name:conference.name}}));
          continue;
        }
        if(matches.length!==1){
          ambiguous++;
          exceptions.push(exceptionRow({conference:{...conference,id:expectedConferenceId},type:"published_roster_ambiguous",localState:`${matches.length} local candidates`,authorityState:`published ${row.school_name}`,recommendedRepair:"resolve school alias ambiguity; do not auto-assign conference",details:{school_name:row.school_name,candidate_team_ids:matches.map(team=>team.team_id),conference_id:expectedConferenceId,conference_name:conference.name}}));
          continue;
        }
        matched++;
      ...
      publishedConferenceAudit.push({conference_id:`${conference.id}-volleyball`,conference_name:conference.name,published_teams:(conference.standings||[]).length,matched_local_teams:matched,aligned_memberships:membershipAligned,unmatched,ambiguous,overall_records_agree:overallAgrees,conference_records_agree:conferenceAgrees,complete_local_membership:unmatched===0&&ambiguous===0&&membershipAligned===matched});
    }
  }
  ...
  for(const final of maxFinals){
    ...
    let type="missing_externally_published_final",localState="no canonical event for published final";
    if(localEvents.some(event=>event.status==="SCHEDULED")){type="stale_scheduled_external_final";localState="canonical event remains SCHEDULED";}
    else if(localEvents.some(event=>event.status==="FINAL")){type="external_final_score_conflict";localState=localEvents.map(event=>`${event.home_score}-${event.away_score}`).join(", ");}
  ...
  const summary={
    ...
    active_arkansas_varsity_volleyball_teams:teamAudit.length,
  ...
  return {summary,conferences:publishedConferenceAudit,teams:teamAudit,exceptions_by_deficiency:groupExceptions(exceptions)};
}
...
export async function collectPublishedVolleyballAuthority(fetchFn=fetch){
  ...
  const discoveryComplete=options.conferences.length>=PUBLISHED_CONFERENCE_MINIMUM;
  const conferences=[],failures=[];
  if(!discoveryComplete) return {conferences,failures,discoveredConferenceCount:options.conferences.length,discoveryComplete};
  ...
  return {conferences,failures,discoveredConferenceCount:options.conferences.length,discoveryComplete};
}
...
export async function collectMaxPrepsVolleyballAuthority(localTeams,{fetchFn=fetch,startDate=SEASON_START,endDate=localDate(new Date())}={}){
  const dates=dateRange(startDate,endDate),matched=[],ambiguous=[],ambiguousFinals=[],failures=[];
  let parsedFinals=0;
  for(let i=0;i<dates.length;i+=AUTHORITY_BATCH_SIZE){
    ...
    const pages=await Promise.all(batch.map(async date=>{
      try{
        ...
        return {date,finals:parseMaxPrepsVolleyballScores(await response.text(),{localDate:date,sourceUrl:response.url||url})};
      }catch(error){
...
export async function loadVolleyballAuditSnapshot(env){
  ...
  return {
    teams:results[0]?.results||[],canonicals:results[1]?.results||[],candidates:results[2]?.results||[],
    d1:{statements:3,rows_read:results.reduce((sum,result)=>sum+rowsRead(result),0),rows_written:writes,per_statement:results.map((result,index)=>({statement:index+1,rows_read:rowsRead(result),rows_written:rowsWritten(result)}))}
  };
}

export async function runVolleyballCompletenessAudit(env,{fetchFn=fetch,now=new Date()}={}){
  // Source preflight happens before the broad D1 read. If the statewide conference
  // directory collapses to the six-conference fallback, stop without touching D1.
  ...
  return {
    ...buildVolleyballCompletenessAudit({teams:snapshot.teams,canonicals:snapshot.canonicals,candidates:snapshot.candidates,published,maxPreps,generatedAt:now.toISOString()}),
    d1:snapshot.d1,
    authority_fetch:{
      discovered_published_conferences:published.discoveredConferenceCount,
      published_conference_failures:published.failures,
      maxpreps_dates:maxPreps.dates.length,maxpreps_parsed_finals:maxPreps.parsedFinals,
      maxpreps_ambiguous_matches:maxPreps.ambiguous.length,maxpreps_failures:maxPreps.failures
    }
  };
}
...
export {
  ...
  SEASON_START,SEASON_END_EXCLUSIVE,MAX_EXCEPTIONS_PER_TYPE,PUBLISHED_CONFERENCE_MINIMUM,
