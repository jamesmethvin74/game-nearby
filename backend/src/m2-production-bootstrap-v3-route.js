import { runM2ProductionBootstrapV3 } from "./m2-production-bootstrap-v3.js";

export const M2_BOOTSTRAP_V3_READY_PATH="/api/v1/_m2-bootstrap-v3-ready-20260903";
export const M2_BOOTSTRAP_V3_RUN_PATH="/api/v1/_m2-bootstrap-v3-run-20260903-1102";
...
export async function maybeHandleM2ProductionBootstrapV3(request,env){
  ...
  if (url.pathname===M2_BOOTSTRAP_V3_READY_PATH) {
    return json({ready:true,release:"m2-production-bootstrap-unique-v3-20260903"});
  }
  if (url.pathname!==M2_BOOTSTRAP_V3_RUN_PATH) return null;
  if (request.headers.get(HEADER)!==HEADER_VALUE) return json({error:"not_found"},404);
  const result=await runM2ProductionBootstrapV3(env);
