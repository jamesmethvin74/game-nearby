...
const REMAINING_CODES=new Set(["WBB","MSO","WSO","WVB"]);
...
function configFromPath(path,prefix){
  ...
  if (!REMAINING_CODES.has(code)) return null;
...
async function sportPreflight(env,config,fetched=null){
  ...
  const existing=externalTeamIds.length ? await env.DB.prepare(`
    ...
    WHERE tei.provider='dragonfly'
      AND tei.external_team_id IN (SELECT value FROM json_each(?))
  `).bind(JSON.stringify(externalTeamIds)).all() : {results:[],meta:{}};
  addMeta(meta,existing);
  const existingByExternal=new Map((existing.results||[]).map(row=>[String(row.external_team_id),{
    teamId:row.team_id,
    schoolId:row.school_id
  }]));

  let crossSchoolCollisions=0;
  let sameSchoolReuse=0;
  const missingSchoolIds=[];
  const missingTeamSchoolIds=[];
  const crossSchoolCollisionDetails=[];
  ...
  for (const entry of validated.entries) {
    ...
    const existingEntry=existingByExternal.get(entry.externalTeamId)||null;
    if (!existingEntry || !teamId || existingEntry.teamId===teamId) continue;
    if (existingEntry.schoolId===schoolId) {
      sameSchoolReuse++;
    } else {
      crossSchoolCollisions++;
      if (crossSchoolCollisionDetails.length<25) {
        crossSchoolCollisionDetails.push({
          externalTeamId:entry.externalTeamId,
          externalSchoolId:entry.externalSchoolId,
          targetSchoolId:schoolId,
          targetTeamId:teamId,
          existingSchoolId:existingEntry.schoolId,
          existingTeamId:existingEntry.teamId
        });
      }
    }
  }
  ...
  const summary={
    ...
    sameSchoolExternalTeamIdReuse:sameSchoolReuse,
    crossSchoolExternalTeamIdCollisions:crossSchoolCollisions,
    crossSchoolCollisionDetails,
...
async function runSport(env,config){
  ...
  return {
    ...
    catalog:{
      ...
      sharedSameSchoolExternalTeamIds:catalog.sharedSameSchoolExternalTeamIds??0,
...
async function verifyAll(env){
  ...
  for (const config of STATEWIDE_HIGH_SCHOOL_SPORTS) {
    ...
    const counts=await env.DB.prepare(`
      SELECT
        COUNT(DISTINCT CASE WHEN src.id IS NOT NULL THEN t.id END) AS exact_source_rows,
        COUNT(DISTINCT CASE WHEN sei.external_school_id IS NOT NULL THEN t.id END) AS school_identity_teams,
      ...
      LEFT JOIN school_external_identities sei
        ON sei.school_id=t.school_id AND sei.provider='dragonfly'
    ...
    sports.push({
      ...
      exactSourceRows:Number(count.exact_source_rows||0),schoolIdentityTeams:Number(count.school_identity_teams||0),
  ...
  const complete=sports.length===6 && sports.every(item=>
    ...
    && item.schoolIdentityTeams===item.expectedTargets
  ...
  return {
    ...
    totals:{
      ...
      schoolIdentityTeams:sports.reduce((n,item)=>n+item.schoolIdentityTeams,0),
...
export async function maybeHandleM2ProductionBootstrapV4(request,env){
  ...
  if (url.pathname===M2_BOOTSTRAP_V4_READY_PATH) {
    return json({ready:true,release:"m2-production-bootstrap-school-scoped-v4-20260903"});
  }
