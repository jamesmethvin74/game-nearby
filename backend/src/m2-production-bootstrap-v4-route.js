...
export const M2_BOOTSTRAP_V4_READY_PATH="/api/v1/_m2-bootstrap-v4-ready-20260903";
const PREFLIGHT_PREFIX="/api/v1/_m2-bootstrap-v4-preflight-20260903-";
const RUN_PREFIX="/api/v1/_m2-bootstrap-v4-run-20260903-";
export const M2_BOOTSTRAP_V4_VERIFY_PATH="/api/v1/_m2-bootstrap-v4-verify-20260903";
...
async function fetchAndValidateSport(config){
  const fetched=await fetchDragonFlyPagedPayload(config.feedUrl,{
    headers:{"user-agent":"LocalBleachersAR-M2-production-bootstrap/4.0","accept":"application/json"}
  });
...
async function sportPreflight(env,config,fetched=null){
  ...
  `).bind(config.teamIdentityProvider,JSON.stringify(externalTeamIds)).all() : {results:[],meta:{}};
  ...
  const summary={
    key:config.key,teamCode:config.teamCode,feedCode:config.feedCode,
    teamIdentityProvider:config.teamIdentityProvider,
    ...
    identityRows:schoolByExternal.size,teamRows:teamBySchool.size,resolvedTargets,
    externalTeamIds:externalTeamIds.length,externalTeamIdentityCollisions:collisions,
    missingSchoolIds:missingSchoolIds.slice(0,25),missingTeamSchoolIds:missingTeamSchoolIds.slice(0,25),d1Meta:meta
  };
...
async function runSport(env,config){
  ...
  if (!preflight.summary.safe) return {status:"PREFLIGHT_FAILED",preflight:preflight.summary};
  ...
  return {
    status:"SUCCESS",key:config.key,teamCode:config.teamCode,preflight:preflight.summary,
    catalog:{
...
async function verifyAll(env){
  ...
  for (const config of STATEWIDE_HIGH_SCHOOL_SPORTS) {
    ...
    const certifiedIds=JSON.stringify([...certifiedTargetSchoolIds(config)]);
    const counts=await env.DB.prepare(`
      SELECT
        COUNT(DISTINCT src.id) AS exact_source_rows,
        COUNT(DISTINCT CASE WHEN tei.provider=? THEN t.id END) AS identity_teams,
        COUNT(DISTINCT CASE WHEN src.enabled=1 THEN src.id END) AS enabled_exact_sources
      ...
      LEFT JOIN team_external_identities tei
        ON tei.team_id=t.id AND tei.provider=?
      WHERE sei.provider='dragonfly'
        AND sei.external_school_id IN (SELECT value FROM json_each(?))
    `).bind(
      config.teamIdentityProvider,config.sport,config.gender,config.season,
      config.teamIdentityProvider,certifiedIds
    ).all();
    addMeta(meta,counts);
    ...
    sports.push({
      ...
      exactSourceRows:Number(count.exact_source_rows||0),identityTeams:Number(count.identity_teams||0),enabledExactSources:Number(count.enabled_exact_sources||0)
    });
  }
  ...
  return {
    ...
    global:globalState,expectedProtectionIndexes:PROTECTION_INDEXES,d1Meta:meta
  };
}

export async function maybeHandleM2ProductionBootstrapV4(request,env){
  const url=new URL(request.url);
  if (request.method!=="GET") return null;
  if (url.pathname===M2_BOOTSTRAP_V4_READY_PATH) {
    return json({ready:true,release:"m2-production-bootstrap-feed-scoped-v4-20260903"});
  }
  if (request.headers.get(HEADER)!==HEADER_VALUE) {
    if (url.pathname.startsWith(PREFLIGHT_PREFIX) || url.pathname.startsWith(RUN_PREFIX) || url.pathname===M2_BOOTSTRAP_V4_VERIFY_PATH) {
  ...
  if (preflightConfig) {
    try {
      ...
      return json({status:"FAILURE",stage:"sport-preflight-v4",error:String(error?.message||error)},409);
    }
  }
  ...
  if (runConfig) {
    try {
      ...
      return json({status:"FAILURE",stage:`sport-run-v4:${runConfig.key}`,error:String(error?.message||error)},409);
    }
  }
  if (url.pathname===M2_BOOTSTRAP_V4_VERIFY_PATH) {
    try {
      ...
      return json({status:"FAILURE",stage:"verification-v4",error:String(error?.message||error)},409);
    }
  }
  return null;
}
