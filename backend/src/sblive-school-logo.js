...
export function parseSBLiveSchoolDirectory(html) {
  ...
  for(const m of text.matchAll(/<a\b([^>]*)>([\s\S]*?)<\/a>/gi)){
    ...
    const match=href.match(/\/high-school\/stats\/arkansas\/schools\/(\d+)-([a-z0-9-]+)(?:[/?#]|$)/i);
    if(!match) continue;
    const schoolId=match[1], slug=match[2].toLowerCase();
    if(seen.has(schoolId)) continue;
    const label=stripTags(m[2]);
    let name=label;
    let city="";
    const loc=label.match(/^(.*?)(?:\s+)([A-Za-z .'-]+),\s*AR$/i);
    if(loc){name=clean(loc[1]);city=clean(loc[2]);}
    if(!name || name.length>100) name=titleCaseSlug(slug);
    seen.add(schoolId);
...
export function parseSBLiveSchoolLogo(html,pageUrl,hints={}){
  const schoolId=pageSchoolId(pageUrl); if(!schoolId) return null;
  const text=normalizeEmbeddedText(html);
  const prefix=`https://assets.scorebooklive.com/r/uploads/production/school/${schoolId}/image/`;
  ...
  for(const m of text.matchAll(re)){
    ...
    let score=50;
    if(/logo|mascot|crest|emblem/.test(lower))score+=50;
    if(schoolTokens(hints).some(t=>lower.includes(t)))score+=20;
    if(/banner|cover|hero|team_photo|team-photo|gallery/.test(lower))score-=60;
    found.push({url,score});
  }
  found.sort((a,b)=>b.score-a.score);
  if(!found.length)return null;
  if(found.length>1&&found[0].score===found[1].score&&found[0].url!==found[1].url)return null;
  if(found[0].score<50)return null;
  return {logoUrl:found[0].url,method:"sblive-school-asset",confidence:found[0].score>=100?1:0.8};
}
