...
async function loadRecordInputs(env, { teamIds = null } = {}) {
  ...
  //
  // Do not filter counts_for_record in SQL. Historical collectors could persist a
  // false zero when no explicit non-record decision existed. The shared record
  // eligibility function must see those rows so it can distinguish a real
  // exhibition/preseason exclusion from a legacy bare zero.
  let canonicalQuery = `
    SELECT DISTINCT ce.*,
