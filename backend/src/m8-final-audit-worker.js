...
const M10_VERIFY_PATH="/api/v1/internal/m10-production-verify-20260915-4e81b7c2";
const M10_VERIFY_EXPIRES_AT=Date.parse("2026-09-16T03:00:00Z");
...
async function runM10Verification(env) {
  const query=await env.DB.prepare(`
    ...
    SELECT
      ...
      SUM(CASE WHEN membership_state='member' AND conference_games=0 THEN 1 ELSE 0 END) AS known_zero_conference_game_members,
...
export default {
  async fetch(request,env,ctx) {
    ...
    const m10Verify=request.method==="GET"
      && url.pathname===M10_VERIFY_PATH
      && Date.now()<=M10_VERIFY_EXPIRES_AT;
    if (m10Verify) {
      try { return await runM10Verification(env); }
      catch (error) {
        console.error("bounded M10 production verification failed",error);
        return auditJson({error:"m10_production_verification_failed",message:String(error?.message||error)},500);
      }
    }
...
export { FINAL_AUDIT_EXPIRES_AT, FINAL_AUDIT_PATH, M10_VERIFY_EXPIRES_AT, M10_VERIFY_PATH };
