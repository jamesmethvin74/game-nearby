...
const FOOTBALL_COLLISION_PROOF_PATH="/api/v1/internal/m15-football-sameday-proof-20260918-31f0c7";
const FOOTBALL_COLLISION_PROOF_FINGERPRINT="m15-football-sameday-v1-20260918";
...
export default {
  async fetch(request,env,ctx) {
    ...
    if(url.pathname===FOOTBALL_COLLISION_PROOF_PATH){
      if(request.method==="GET") return auditJson({status:"READY",fingerprint:FOOTBALL_COLLISION_PROOF_FINGERPRINT},200,{integrity:true});
      if(request.method!=="POST") return auditJson({error:"not_found"},404,{integrity:true});
      const body=await request.json().catch(()=>({}));
      if(body.fingerprint!==FOOTBALL_COLLISION_PROOF_FINGERPRINT) return auditJson({error:"not_found"},404,{integrity:true});
      const result=await runStatewideIntegrityGate(env,{now:new Date(),reason:"football-sameday-proof"});
      return auditJson({status:"EXECUTED",fingerprint:FOOTBALL_COLLISION_PROOF_FINGERPRINT,result},200,{integrity:true});
    }
