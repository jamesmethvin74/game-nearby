...
export function rowCountsForRecord(row = {}) {
  // Fresh parser output can carry an explicit boolean false before persistence.
  if (row.countsForRecord === false) return false;

  // A named non-count contest remains excluded even if an old materialization
  // later lost the original boolean provenance.
  ...
  // Arkansas high-school basketball games before the AAA first official contest
  // date stay visible on schedules but must not enter official W/L records.
  if (highSchoolBasketballPreseason(row)) return false;

  if (Number(row.counts_for_record) !== 0) return true;

  // Certified DragonFly normalization deliberately writes zero for an explicit
  // exhibition/preseason state. That provider-level zero is authoritative.
  if (clean(row.parser_type).toLowerCase() === "dragonfly-public") return false;

  // Legacy collector versions could persist zero simply because a parser omitted
  // countsForRecord. A bare zero with no non-record evidence must not suppress a
  // scored ordinary game forever. Recover it as record-counting at read/rebuild.
  return true;
}
