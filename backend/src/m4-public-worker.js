...
const APPROVED_BOOTSTRAP_BATCH5_PATH = "/api/v1/m4/bootstrap-approved-b5";
...
function authorizedBatch5(request, env) {
  return Boolean(env.M4_BATCH5_TOKEN) && request.headers.get("x-m4-batch5-token") === env.M4_BATCH5_TOKEN;
}
...
async function runApprovedBootstrapBatch5(request, env, ctx) {
  if (!authorizedBatch5(request, env)) return privateJson({ error:"not_found" }, 404);
  const result = await runScopedCadence({
    core,
    env,
    ctx,
    controller:null,
    plan:{
      kind:"m4-college-initial-ingestion-approved-batch5",
      scope:"college-bootstrap",
      season:COLLEGE_BOOTSTRAP_SEASON
    }
  });
...
function batch5Readiness(request, env) {
  if (!authorizedBatch5(request, env)) return privateJson({ error:"not_found" }, 404);
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === APPROVED_BOOTSTRAP_BATCH5_PATH) {
      if (request.method === "HEAD") return batch5Readiness(request, env);
      if (request.method === "POST") return runApprovedBootstrapBatch5(request, env, ctx);
    }
...
export {
  APPROVED_BOOTSTRAP_BATCH5_PATH,
