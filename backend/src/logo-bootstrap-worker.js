...
import { readHarrisonMountainHomeAudit, planHarrisonMountainHomeRepair, executeHarrisonMountainHomeRepair, HARRISON_MH_REPAIR_FINGERPRINT } from "./harrison-mountain-home-audit.js";
...
export const HARRISON_MH_VERSION_PATH = "/api/v1/internal/harrison-mountain-home-version-42f7c9b1";
export const HARRISON_MH_PLAN_PATH = "/api/v1/internal/harrison-mountain-home-plan-42f7c9b1";
export const HARRISON_MH_EXECUTE_PATH = "/api/v1/internal/harrison-mountain-home-execute-42f7c9b1";
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (path.startsWith("/api/v1/internal/harrison-mountain-home-") && Date.now() > HARRISON_MH_AUDIT_EXPIRES_AT) return privateJson({ error:"not_found" }, 404);
    if (request.method === "GET" && path === HARRISON_MH_VERSION_PATH) return privateJson({ marker:HARRISON_MH_REPAIR_FINGERPRINT, d1_access:false });
    ...
    if (request.method === "GET" && path === HARRISON_MH_PLAN_PATH) {
      try { return privateJson(await planHarrisonMountainHomeRepair(env)); }
      catch (error) { return privateJson({ error:"harrison_mountain_home_plan_failed", message:String(error?.message || error) }, 500); }
    }
    if (request.method === "POST" && path === HARRISON_MH_EXECUTE_PATH) {
      const input=await options(request);
      try { return privateJson(await executeHarrisonMountainHomeRepair(env,{fingerprint:input.fingerprint})); }
      catch (error) { return privateJson({ error:"harrison_mountain_home_execute_failed", message:String(error?.message || error) }, 409); }
    }
