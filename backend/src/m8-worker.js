...
function jsonFrom(upstream, body, extraHeaders = {}) {
...
export async function rewritePublicRead(request, response, env) {
  ...
  if (path === "/api/v1/schools") {
    ...
    return jsonFrom(response, body, { "x-localbleachers-logo-delivery": LOGO_DELIVERY_RELEASE });
  }
  ...
  if (schedulePath) {
    ...
    return jsonFrom(response, correctSchedulePayload(body), {
      "x-localbleachers-result-orientation": RESULT_ORIENTATION_RELEASE
    });
  }
...
<style>
:root{color-scheme:dark;font-family:system-ui,-apple-system,"Segoe UI",sans-serif;background:#071524;color:#f7f9fc}*{box-sizing:border-box}body{margin:0;background:#071524}.wrap{max-width:780px;margin:auto;padding:22px 16px 50px}.eyebrow{color:#64abf7;font-weight:900;letter-spacing:.16em;font-size:.8rem}.title{font-size:1.8rem;font-weight:900;margin:5px 0 4px}.sub{color:#9badc4;line-height:1.45;margin-bottom:18px}.card{background:#0d2035;border:1px solid #29425f;border-radius:20px;padding:16px;margin:14px 0}.record{display:grid;grid-template-columns:1fr 1fr;gap:10px}.stat{background:#132941;border:1px solid #29425f;border-radius:15px;padding:14px}.label{color:#9badc4;font-size:.7rem;font-weight:900;letter-spacing:.1em}.value{font-size:1.5rem;font-weight:900;margin-top:4px}.game{display:flex;justify-content:space-between;gap:12px;padding:12px 0;border-top:1px solid #29425f}.score{font-weight:900;white-space:nowrap}.ok{color:#73d49f}.bad{color:#ff8d8d}.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:10px}.school{display:grid;grid-template-columns:54px 1fr;gap:11px;align-items:center;background:#0d2035;border:1px solid #29425f;border-radius:16px;padding:10px}.logo{width:54px;height:54px;border-radius:12px;background:#fff;display:grid;place-items:center;overflow:hidden;color:#0d2035;font-weight:900}.logo img{width:100%;height:100%;object-fit:contain;padding:3px}.name{font-weight:850;font-size:.9rem}.meta{color:#9badc4;font-size:.75rem;margin-top:2px}.section{font-size:1.05rem;font-weight:900;margin:24px 2px 10px}.banner{border:1px solid #31557d;background:#102a46;border-radius:14px;padding:11px 13px;color:#b9cee5;font-size:.8rem}.count{color:#9badc4;font-size:.8rem;margin-left:4px}#error{white-space:pre-wrap}</style>
</head>
<body><main class="wrap">
<div class="eyebrow">BRANCH PREVIEW</div>
<div class="title">College trust fix</div>
...
<div class="section">Arkansas colleges <span id="count" class="count"></span></div>
<div id="colleges" class="grid"></div>
<div id="error" class="bad"></div>
</main>
<script>
function esc(v){return String(v==null?"":v).replace(/[&<>"']/g,function(c){return {"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#39;"}[c];});}
function scoreText(g){if(g.status!=="FINAL"||g.team_score==null||g.opponent_score==null)return "";return String(g.result||"")+" "+String(g.team_score)+"-"+String(g.opponent_score);}
async function load(){
  var pair=await Promise.all([fetch("/api/v1/schools",{cache:"no-store",headers:{accept:"application/json"}}),fetch("/api/v1/schools/uark/schedule",{cache:"no-store",headers:{accept:"application/json"}})]);
  var schoolsRes=pair[0], scheduleRes=pair[1];
  if(!schoolsRes.ok||!scheduleRes.ok)throw new Error("API "+schoolsRes.status+"/"+scheduleRes.status);
  var schools=await schoolsRes.json(), schedule=await scheduleRes.json();
  var statuses=Array.isArray(schedule.team_statuses)?schedule.team_statuses:[];
  var football=statuses.find(function(x){return x.sport==="football"&&String(x.gender||"").toLowerCase()==="men";})||statuses.find(function(x){return x.sport==="football";});
  var games=(Array.isArray(schedule.games)?schedule.games:[]).filter(function(x){return x.sport==="football"&&x.status==="FINAL";});
  var overall=football&&football.overall_record?football.overall_record:"N/A";
  var overallClass=overall==="1-1"?"ok":"bad";
  var html='<div class="record"><div class="stat"><div class="label">ARKANSAS OVERALL</div><div class="value '+overallClass+'">'+esc(overall)+'</div></div><div class="stat"><div class="label">READ RELEASE</div><div class="value" style="font-size:.9rem">'+esc(scheduleRes.headers.get("x-localbleachers-result-orientation")||"missing")+'</div></div></div><div style="margin-top:12px">';
  games.forEach(function(g){html+='<div class="game"><div><strong>'+(g.home_away==="home"?"vs. ":"at ")+esc(g.opponent)+'</strong></div><div class="score">'+esc(scoreText(g))+'</div></div>';});
  html+='</div>';
  document.getElementById("arkansas").innerHTML=html;
  var colleges=(Array.isArray(schools.schools)?schools.schools:[]).filter(function(x){return x.level==="college";}).sort(function(a,b){return String(a.name).localeCompare(String(b.name));});
  document.getElementById("count").textContent=String(colleges.length)+" colleges";
  var cards="";
  colleges.forEach(function(s){var initial=esc(String(s.name||"?").charAt(0));var image=s.logo_url?'<img src="'+esc(s.logo_url)+'" alt="" onload="this.previousElementSibling.style.display=\'none\'" onerror="this.style.display=\'none\'">':"";cards+='<div class="school"><div class="logo"><span>'+initial+'</span>'+image+'</div><div><div class="name">'+esc(s.name)+'</div><div class="meta">'+esc(s.mascot||"")+' · '+esc(s.city||"")+'</div></div></div>';});
  document.getElementById("colleges").innerHTML=cards;
}
load().catch(function(e){document.getElementById("error").textContent="Preview failed: "+String(e&&e.message?e.message:e);});
</script></body></html>`;
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === "GET" && url.pathname === COLLEGE_TRUST_PREVIEW_PATH) {
    ...
    if (wantsResultGaps) {
      ...
      if (!Array.isArray(report?.teams)) {
        return jsonFrom(upstream, {
          error: "m8_result_gap_audit_unavailable",
          message: "Truthful coverage snapshot did not include team evidence."
        }, { "x-localbleachers-m8-audit": "result-gaps-v1" });
      }
      return jsonFrom(upstream, buildResultGapAudit(report), { "x-localbleachers-m8-audit": "result-gaps-v1" });
    }

    const upstream = await app.fetch(request, env, ctx);
    return rewritePublicRead(request, upstream, env);
  },
...
export { LOGO_DELIVERY_RELEASE, RESULT_GAP_VIEW, RESULT_ORIENTATION_RELEASE };