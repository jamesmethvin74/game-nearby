...
import { parseRankOnePublicHtml } from "./rankone-public-html.js";

const EXTERNAL_HTML_PARSERS = new Set(["prestosports-rss", "institutional-table", "rankone-public"]);

export function isCollegeRuntimeParser(parserType) {
  return EXTERNAL_HTML_PARSERS.has(String(parserType || ""));
}
...
export async function parseCollegeSourceBody(body,source,{HTMLRewriterClass=globalThis.HTMLRewriter}={}) {
  ...
  if (source?.parser_type==="rankone-public") return parseRankOnePublicHtml(body,source,HTMLRewriterClass);
  return null;
}
