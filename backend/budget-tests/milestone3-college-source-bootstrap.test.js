...
test("M3 source bootstrap can represent exactly the 84 currently parser-ready targets",()=>{
...
test("M3 source materialization is one set-based disabled insert with no schedule side effects",()=>{
  ...
  assert.equal(Number(result.changes),84);
  assert.equal(Number(db.prepare("SELECT COUNT(*) n FROM sources WHERE id LIKE 'college-%'").get().n),84);
