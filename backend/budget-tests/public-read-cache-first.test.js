...
test("school-level schedule cache is versioned away from pre-unified team-status payloads", () => {
  assert.equal(SCHEDULE_CACHE_VERSION, "school-dedupe-v4");
  ...
  assert.match(new URL(descriptor.freshKey.url).pathname, /\/fresh\/schedule\/school-dedupe-v4\/api\/v1\/schools\/uca\/schedule$/);
...
test("team schedule cache is also versioned while record cache remains stable", () => {
  ...
  assert.match(new URL(schedule.freshKey.url).pathname, /\/fresh\/schedule\/school-dedupe-v4\/api\/v1\/teams\/t1\/schedule$/);
