...
test("official final-result pass is limited to the evening and morning reconciliation windows",()=>{
  ...
  assert.equal(shouldRunOfficialFinalResults({kind:"friday-football-results"}),false);
  assert.equal(shouldRunOfficialFinalResults({kind:"weekly-catalog-maintenance"}),false);
});

test("official final-result selector is high-school, official-source, three-sport, and hard capped",()=>{
  const policy=scopePolicy({scope:"high-school-final-results",activeResultMinutes:120});
  assert.equal(policy.maxSources,16);
  assert.equal(OFFICIAL_FINAL_RESULTS_MAX_SOURCES_PER_RUN,16);
