...
test("truthful v3 contract never equates presence with completeness", () => {
  assert.equal(AUDIT_CONTRACT_V3.rule, "Present is not Complete.");
  assert.match(AUDIT_CONTRACT_V3.retained_source_rule, /disabled statewide sources/);
  const report = audit([
...
test("v3 SQL preserves retained source evidence instead of filtering on enabled=1", async () => {
