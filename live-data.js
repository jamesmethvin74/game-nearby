(() => {
  ...
  function replacePresentationStatusesInMap(target, statuses = [], resolutions = [], requestedSchoolIds = []) {
    ...
    const clearIds = new Set(requestedSchoolIds.map(String).filter(Boolean));
    for (const [canonical, aliases] of aliasesByCanonical) {
      clearIds.add(canonical);
      aliases.forEach(alias => clearIds.add(alias));
    }
